...
import com.doctor.sun.ui.fragment.PnewMedicalRecordFragment;
...
public class TokenCallback {
    ...
    private static AuthModule api = Api.of(AuthModule.class);
        ...
        profileModule.patientProfile().enqueue(new ApiCallback<PatientDTO>() {
            ...
            protected void handleResponse(PatientDTO response) {
                ...
                if (response == null) {
                    ...
                    if (response.getInfo().getRecordNames().size() > 0) {
                        Intent i = PMainActivity2.makeIntent(context);
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                            context.finishAffinity();
                        } else {
                        ...
                        to.setClass(context, PnewMedicalRecordFragment.class);
    ...
    public static void handleToken(Token token) {
        ...
        Config.putString(Constants.ACCOUNT,token.getAccount().getYunxinAccid());
        /*AppContext.getInstance().setAccount(token.getAccount().getYunxinAccid());*/
